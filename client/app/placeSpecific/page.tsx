...
useEffect(() => {
  ...
  if (!token || !idURL) {
    console.error("El token o idURL no están definidos");
    return;
  }
  const fetchCity = async () => {
    try {
      const response = await axios.get(`https://no-country-back.onrender.com/api/city/${idURL}`, {
        headers: {
...
<div className=' flex  flex-col md:flex-row p-2 '>
     <Image
      ...
      /> 
         ...
         <div className="flex  flex-col justify-center   ">
          ...
          <h2 className='font-bold '> {lugares.name.charAt(0).toUpperCase() + lugares.name.slice(1).toLowerCase()}</h2>     
  ...
  <Link href={`/descripcion/${lugares.id}`}>Ver más</Link>
</button>
