...
const CarrouselExplore: React.FC = () => {
  ...
  const handleClickPrev = () => {
    const prevIndex = startIndex - 1 >= 0 ? startIndex - 1 : actividades.length - 3;
  ...
  return (
    <>
      <div className=" mb-5 mt-1">
        <h1 className="text-center font-aeonik  text-2xl">Explorar actividades</h1>
      </div>
        ...
        <div className="mx-auto bg-soft-green">
          ...
          <div className="relative overflow-hidden ">
            <div className="flex flex-row p-10 " style={{ transform: `translateX(-${startIndex * 33.33}%)` }}>
              {actividades.slice(startIndex, startIndex + 3).map(actividad => (
            ...
            {showPrevButton &&
              <button className=" text-white absolute top-1/2 transform -translate-y-10  left-18 " onClick={handleClickPrev}>
            ...
            <button className="absolute top-1/2 text-white transform -translate-y-10  right-0" onClick={handleClickNext}>
